...
exports.NovaSettingsPanel = void 0;
const vscode = __importStar(require("vscode"));
class NovaSettingsPanel {
    static viewType = 'nova.settings';
    static currentPanel;
    _panel;
    _context;
    _disposables = [];
    ...
    static show(context) {
        ...
        const panel = vscode.window.createWebviewPanel(NovaSettingsPanel.viewType, 'Nova SDK Settings', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    _getCurrentSettings() {
        ...
        return {
            executablePath: cfg.get('executablePath', 'nova'),
            sdkServerPath: cfg.get('sdkServerPath', ''),
            runArgs: (cfg.get('runArgs', []) || []).join(' '),
            optimizationLevel: cfg.get('optimizationLevel', '-O1'),
            stripDebugInfo: cfg.get('stripDebugInfo', false),
            lspEnabled: cfg.get('lsp.enabled', true),
            lspPath: cfg.get('lsp.path', ''),
            lspTrace: cfg.get('lsp.trace', 'off'),
            mcpEnabled: cfg.get('mcp.enabled', true),
            mcpPath: cfg.get('mcp.path', ''),
            mcpSandbox: cfg.get('mcp.sandbox', true),
            terminalShellIntegration: cfg.get('terminal.shellIntegration', true),
            taskfileAutoDetect: cfg.get('taskfile.autoDetect', true),
            taskfilePath: cfg.get('taskfile.path', 'taskfile.nini'),
            traceEnabled: cfg.get('trace.enabled', false),
            traceChannels: cfg.get('trace.channels', 'all'),
        };
    }
        ...
        switch (msg.type) {
            ...
            case 'updateSetting':
                if (msg.key && msg.value !== undefined) {
                    ...
                    if (msg.key === 'runArgs') {
                        val = msg.value.split(/\s+/).filter(Boolean);
                    }
...
exports.NovaSettingsPanel = NovaSettingsPanel;
