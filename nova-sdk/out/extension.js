...
const themeCustomizer_1 = require("./themeCustomizer");
const iconCustomizer_1 = require("./iconCustomizer");
const toolboxProvider_1 = require("./toolboxProvider");
/* ================================================================
 * Configuration helpers
 * ================================================================ */
function getNovaPath() {
    return vscode.workspace.getConfiguration('nova').get('executablePath', 'nova');
}
function getOptLevel() {
    return vscode.workspace.getConfiguration('nova').get('optimizationLevel', '-O1');
}
function getStripFlag() {
    return vscode.workspace.getConfiguration('nova').get('stripDebugInfo', false);
}
function getRunArgs() {
    return vscode.workspace.getConfiguration('nova').get('runArgs', []);
}
function getTaskfilePath() {
    return vscode.workspace.getConfiguration('nova').get('taskfile.path', 'taskfile.nini');
}
 ...
 */
let _novaToolTerminal;
function getNovaToolTerminal() {
...
function registerCommands(context) {
    /* --- Run current file --- */
    context.subscriptions.push(vscode.commands.registerCommand('nova.runFile', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('nova.compileFile', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('nova.disassemble', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            return;
        }
    ...
    /* --- Show AST --- */
    context.subscriptions.push(vscode.commands.registerCommand('nova.showAST', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            return;
        }
    ...
    /* --- Explain error code --- */
    context.subscriptions.push(vscode.commands.registerCommand('nova.explainError', async () => {
    ...
    /* --- Show version --- */
    context.subscriptions.push(vscode.commands.registerCommand('nova.showVersion', () => {
        const nova = getNovaPath();
    ...
    context.subscriptions.push(vscode.commands.registerCommand('nova.openTerminal', () => {
        const nova = getNovaPath();
        const useShell = vscode.workspace.getConfiguration('nova')
            .get('terminal.shellIntegration', true);
    ...
    /* --- Open settings panel --- */
    context.subscriptions.push(vscode.commands.registerCommand('nova.openSettings', () => {
        settingsPanel_1.NovaSettingsPanel.show(context);
    }));
    /* --- Theme customizer --- */
    context.subscriptions.push(vscode.commands.registerCommand('nova.customizeTheme', () => {
        themeCustomizer_1.ThemeCustomizerPanel.show(context.extensionUri);
    }));
    /* --- Icon customizer --- */
    context.subscriptions.push(vscode.commands.registerCommand('nova.customizeIcons', () => {
        iconCustomizer_1.IconCustomizerPanel.show(context.extensionUri);
    }));
    /* --- Run task --- */
    context.subscriptions.push(vscode.commands.registerCommand('nova.runTask', async () => {
        const nova = getNovaPath();
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
...
class NovaTaskProvider {
    ...
    async detectTasks() {
        ...
        for (const folder of workspaceFolders) {
            ...
            while ((match = nameRegex.exec(content)) !== null) {
                ...
                const descRegex = new RegExp(`\\[task:${taskName}\\][\\s\\S]*?description\\s*=\\s*(.+)`, 'm');
    ...
    createTask(taskName, taskfile, folder, description) {
        const nova = getNovaPath();
        const definition = {
        ...
        const task = new vscode.Task(definition, folder ?? vscode.TaskScope.Workspace, description || taskName, 'Nova', exec, '$nova');
 ...
 */
function resolveSdkServer(context) {
    ...
    const novaResolved = novaPath === 'nova'
        ? (() => { try {
            return cp.execSync('which nova 2>/dev/null', { encoding: 'utf-8' }).trim();
        }
        catch {
            return '';
        } })()
...
function startLspClient(context, outputChannel) {
    ...
    if (!serverPath) {
        outputChannel.appendLine('Nova LSP: nova-sdk-server not found — diagnostics unavailable. ' +
    ...
    const clientOptions = {
        ...
        synchronize: {
            fileEvents: vscode.workspace.createFileSystemWatcher('**/*.{n,m,no,nini,ni}'),
        },
        outputChannel,
        traceOutputChannel: outputChannel,
    };
    lspClient = new node_1.LanguageClient('nova-sdk-server', 'Nova Language Server', serverOptions, clientOptions);
 ...
 */
function configureMcpServer(context, outputChannel) {
    const enabled = vscode.workspace.getConfiguration('nova')
        .get('mcp.enabled', true);
    if (!enabled) {
    ...
    const serverPath = resolveSdkServer(context);
    if (!serverPath) {
        outputChannel.appendLine('Nova MCP: nova-sdk-server not found — AI tools unavailable. ' +
            'Build nova-sdk and ensure nova-sdk-server is on PATH or set ' +
            'nova.sdkServerPath in settings.');
        return;
    }
    const sandbox = vscode.workspace.getConfiguration('nova')
        .get('mcp.sandbox', true);
    ...
    outputChannel.appendLine(`Nova MCP: configured in ${mcpFile} (sandbox=${sandbox ? 'ON' : 'OFF'}, binary=${serverPath})`);
}
class NovaTerminalProfileProvider {
 ...
 * ================================================================ */
class NovaDashboardViewProvider {
    static viewType = 'nova.dashboardView';
    _context;
    constructor(context) {
    ...
    resolveWebviewView(webviewView) {
        ...
        const nova = getNovaPath();
        const lspEnabled = vscode.workspace.getConfiguration('nova').get('lsp.enabled', true);
        const mcpEnabled = vscode.workspace.getConfiguration('nova').get('mcp.enabled', true);
...
function activate(context) {
    ...
    if (fileAssoc['*.n'] && fileAssoc['*.n'] !== 'nova') {
        ...
        vscode.window.showWarningMessage(msg, 'Fix Now').then(choice => {
            if (choice === 'Fix Now') {
                vscode.workspace.getConfiguration('files').update('associations', { ...fileAssoc, '*.n': 'nova' }, vscode.ConfigurationTarget.Workspace);
            }
        });
    }
    /* Status bar */
    ...
    const autoDetect = vscode.workspace.getConfiguration('nova')
        .get('taskfile.autoDetect', true);
    if (autoDetect) {
        const taskProvider = new NovaTaskProvider();
        context.subscriptions.push(vscode.tasks.registerTaskProvider(NovaTaskProvider.type, taskProvider));
    }
    /* Terminal profile */
    context.subscriptions.push(vscode.window.registerTerminalProfileProvider('nova.terminal', new NovaTerminalProfileProvider()));
    /* Nova sidebar dashboard */
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('nova.dashboardView', new NovaDashboardViewProvider(context)));
    /* Nova toolbox tree view */
    const toolboxProvider = new toolboxProvider_1.NovaToolboxProvider(context);
    ...
    /* Toolbox commands */
    context.subscriptions.push(vscode.commands.registerCommand('nova.refreshToolbox', () => {
        toolboxProvider.refresh();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('nova.runNamedTask', async (taskName) => {
        const nova = getNovaPath();
        const terminal = getNovaToolTerminal();
        terminal.sendText(`${nova} task ${taskName}`);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('nova.runBuiltinTool', async (toolName) => {
        const nova = getNovaPath();
        const editor = vscode.window.activeTextEditor;
        const activeFile = editor?.document.fileName;
        const cmdParts = [nova, toolName];
    ...
    context.subscriptions.push(vscode.commands.registerCommand('nova.runUserScript', async (arg) => {
        ...
        if (typeof arg === 'string') {
            ...
            scriptPath = arg.data.scriptPath || '';
        }
        else {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('nova.cdToFolder', async (resource) => {
        let targetDir;
        if (resource) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('nova.addUserScript', async () => {
        const wsRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (!wsRoot) {
            vscode.window.showWarningMessage('No workspace folder open.');
            return;
        }
        const choice = await vscode.window.showQuickPick(['Create new script', 'Link existing file'], { placeHolder: 'Add a script to the toolbox' });
        if (choice === 'Create new script') {
            ...
            fs.writeFileSync(filePath, `-- ${name}.n — Nova toolbox script\n` +
    ...
    const taskfileWatcher = vscode.workspace.createFileSystemWatcher('**/taskfile.nini');
