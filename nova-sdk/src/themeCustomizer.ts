...
/* ------------------------------------------------------------------ */
/*  Token categories the UI exposes                                    */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/*  Syntax profiles — pre-built palettes                               */
/* ------------------------------------------------------------------ */
...
interface SyntaxProfile {
    ...
    name: string;
    description: string;
    /** Ordered by TOKEN_CATEGORIES index */
    colors: string[];
    fontStyles?: (string | undefined)[];
}

const SYNTAX_PROFILES: SyntaxProfile[] = [
    {
        id: 'phosphor',
        name: 'Nova Phosphor',
...
export class ThemeCustomizerPanel {
    public static readonly viewType = 'novaThemeCustomizer';
    private static _instance: ThemeCustomizerPanel | undefined;
    ...
    private _getCurrentOverrides(): Record<string, { foreground?: string; fontStyle?: string }> {
        const cfg = vscode.workspace.getConfiguration('editor');
        const custom = cfg.get<Record<string, unknown>>('tokenColorCustomizations', {});
        const rules = (custom as { textMateRules?: Array<{ scope: string | string[]; settings: { foreground?: string; fontStyle?: string } }> })
            .textMateRules || [];

        const map: Record<string, { foreground?: string; fontStyle?: string }> = {};
    ...
    private _getCustomProfiles(): SyntaxProfile[] {
        const cfg = vscode.workspace.getConfiguration('nova');
        const saved = cfg.get<Array<{ id: string; name: string; description: string; colors: string[]; fontStyles?: (string | undefined)[] }>>('customSyntaxProfiles', []);
        return saved.map(p => ({
            id: p.id,
            name: p.name,
    ...
    private _getAllProfiles(): SyntaxProfile[] {
        return [...SYNTAX_PROFILES, ...this._getCustomProfiles()];
    }

    /* ---- Build per-category color from current overrides ---- */
    private _getCategoryColors(): Array<{ foreground: string; fontStyle: string }> {
    ...
    private async _applyTokenColors(
        colors: Array<{ scopes: string[]; foreground: string; fontStyle?: string }>,
    ): Promise<void> {
        const cfg = vscode.workspace.getConfiguration('editor');
        const current = cfg.get<Record<string, unknown>>('tokenColorCustomizations', {});
        const rules: Array<{ scope: string[]; settings: { foreground?: string; fontStyle?: string } }> = [];

        for (const entry of colors) {
            const settings: { foreground?: string; fontStyle?: string } = {};
            if (entry.foreground) { settings.foreground = entry.foreground; }
    ...
    private async _clearOverrides(): Promise<void> {
        const cfg = vscode.workspace.getConfiguration('editor');
    ...
    private async _handleMessage(msg: {
        ...
        switch (msg.type) {
            ...
            case 'saveProfile': {
                ...
                const newProfile: SyntaxProfile = {
                    id,
                    name,
                ...
                const cfg = vscode.workspace.getConfiguration('nova');
                const existing = cfg.get<SyntaxProfile[]>('customSyntaxProfiles', []);
            ...
            case 'deleteProfile': {
                if (!msg.profileId) { break; }
                const cfg = vscode.workspace.getConfiguration('nova');
                const profiles = cfg.get<SyntaxProfile[]>('customSyntaxProfiles', []);
    ...
    private _getHtml(): string {
        const profilesJson = JSON.stringify(
            SYNTAX_PROFILES.map((p) => ({ id: p.id, name: p.name, description: p.description, colors: p.colors })),
        ).replace(/</g, '\\u003c');

        const categoriesJson = JSON.stringify(
            TOKEN_CATEGORIES.map((c) => ({ label: c.label, description: c.description })),
        ).replace(/</g, '\\u003c');
